...
意味	入力	出力
--------------------
追加されたテキスト	@<code>|@@<nop>{}<ins>{xxx}|	@<ins>{xxx}
プログラムコード	@<code>|@@<nop>{}<code>{xxx}|	@<code>{xxx}
ファイル名		@<code>|@@<nop>{}<file>{xxx}|	@<file>{xxx}
ユーザ入力文字列	@<code>|@@<nop>{}<userinput>{xxx}|	@<userinput>{xxx}
//}

追加されたテキストを表す「@<code>|@@<nop>{}<ins>{}|」は、通常の本文では下線になりますが、プログラムコード（@<code>|//list{ ... //}|）やターミナル（@<code>|//terminal{ ... //}|）の中では太字になります。
...
加えて両者ではシングルクォートとバッククォートの表示が異なり、たとえば「@<code>|@@<nop>{}<tt>{'`}|」は「@<tt>{'`}」と表示され、「@<code>|@@<nop>{}<code>{'`}|」は「@<code>{'`}」と表示されます（Starter拡張）@<fn>{fn-2byje}。

//footnote[fn-2byje][使用する等幅フォントによっては同じように表示されます。]

同じような理由で、ファイル名を表すには「@<code>|@@<nop>{}<file>{}|」を使ってください。
...
第2引数に説明文字列を指定した場合、Re:VIEWでは「図：」という接頭辞がつきます。
しかし@<code>|//indepimage|の用途を考えると、この接頭辞は邪魔でしょう。
そこでStarterでは、このような接頭辞をつけないようにしました。
もし「図：」が必要なら自分で説明文字列につけてください。
...
その場合、必ずフェンス記法@<fn>{fn-mzjk0}を使って「@<code>$@@<nop>{}<term>@<b>{|}@@<nop>{}<xxx>{...}@<b>{|}$」のように書いてください（これは実装上の都合です）。
「@<code>$@@<nop>{}<term>@<b>|{|@@<nop>{}<xxx>{...}@<b>|}|$」だとエラーになります。

//footnote[fn-mzjk0][Re:VIEWでは「@<code>$@@<nop>{}<foo>@<b>{|}...@<b>{|}$」や「@<code>|@@<nop>{}<foo>@<b>{$}...@<b>{$}|」のような書き方を「フェンス記法」と呼んでいます。]
...
//footnote[fn-bz0je][索引の機能を@<LaTeX>{}に任せているため、用語のチェックが難しいのです。]
...
たとえば「Apple」と「アップル」のどちらの表記にするかまだ決まってなくても、本文に「@<code>|@@<nop>{}<w>{apple}|」と書いておけば、あとから辞書の中身を変えるだけでどちらの表記にも対応できます。
